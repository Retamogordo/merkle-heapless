...
//    use crate::foo::{Foo};
    ...
    fn fail_4layers_std_hash_bad_word() {
        ...
        let mut mt = HeaplessTree::<BRANCH_FACTOR, HEIGHT, HEIGHT, StdHash>::try_from(&[
//            let mut mt = HeaplessTree::<BRANCH_FACTOR, HEIGHT, HEIGHT, StdHash, Proof<BRANCH_FACTOR, HEIGHT, StdHash>>::try_from(&[
...
//         let mut mt = HeaplessTree::<4, 8, 8, StdHash>::try_from(&[
//             b"apple", b"banana", b"kiwi", b"kotleta",
//         ]);
//         let word_index = 7;
...
//         let mt = HeaplessTree::<3, 1, 1, Blake2_256Hash>::try_from(&[
//             b"apple",
//         ]);
...
//             "cherry",
//         ];
//         let mt = HeaplessTree::<BRANCH_FACTOR, HEIGHT, HEIGHT, StdHash>::try_from(
//             &words.iter().map(|w| w.as_bytes()).collect::<Vec<_>>()
//         );
...
//         const HEIGHT: usize = 4;
//         const BRANCH_FACTOR: usize = 2;
//         let cmt = CompactableHeaplessTree::<BRANCH_FACTOR, HEIGHT, HEIGHT, StdHash>::try_from(&[
//             b"apple", b"banana", b"kiwi", b"kotleta",
//         ])
//         .unwrap();
...
//         let mut cmt = CompactableHeaplessTree::<BRANCH_FACTOR, HEIGHT, HEIGHT, StdHash>::try_from(
//             &words.iter().map(|w| w.as_bytes()).collect::<Vec<_>>()
//         )
//         .unwrap();
...
//         let mut mt = HeaplessTree::<BRANCH_FACTOR, NEW_HEIGHT, NEW_HEIGHT, StdHash>::try_from_leaves(
//             &mt.leaves()
//         )
//         .unwrap();
...
//         ];
//         let cmt1 = CompactableHeaplessTree::<BRANCH_FACTOR, HEIGHT_1, MAX_PROOF_HEIGHT, StdHash>::try_from(
...
//             "kiwi", "kotleta",
//         ];
//         let cmt2 = CompactableHeaplessTree::<BRANCH_FACTOR, HEIGHT_2, MAX_PROOF_HEIGHT, StdHash>::try_from(
...
//             "apple", "apricot", "banana", "cherry", "blueberry"
//         ];
//         let mut cmt1 = CompactableHeaplessTree::<BRANCH_FACTOR, HEIGHT_1, MAX_PROOF_HEIGHT, StdHash>::try_from(
//             &words1.iter().map(|w| w.as_bytes()).collect::<Vec<_>>()
//         )
//         .unwrap();
...
//         ];
//         let mut cmt2 = CompactableHeaplessTree::<BRANCH_FACTOR, HEIGHT_2, MAX_PROOF_HEIGHT, StdHash>::try_from(
//             &words2.iter().map(|w| w.as_bytes()).collect::<Vec<_>>()
//         )
//         .unwrap();
...
//             "apple", "apricot", "banana", "cherry", "blueberry"
//         ];
//         let cmt1 = CompactableHeaplessTree::<BRANCH_FACTOR, HEIGHT_1, HEIGHT_1, StdHash>::try_from(
//             &words1.iter().map(|w| w.as_bytes()).collect::<Vec<_>>()
//         )
//         .unwrap();
...
//             "kiwi", "kotleta",
//         ];
//         let cmt2 = CompactableHeaplessTree::<BRANCH_FACTOR, HEIGHT_2, HEIGHT_2, StdHash>::try_from(
//             &words2.iter().map(|w| w.as_bytes()).collect::<Vec<_>>()
//         )
//         .unwrap();
...
//         let cmt = CompactableHeaplessTree::<BRANCH_FACTOR, FIRST_PEAK_HEIGHT, 5, StdHash>::try_from(
//             &words1.iter().map(|w| w.as_bytes()).collect::<Vec<_>>()
//         )
//         .unwrap();
...
//         let cmt1 = CompactableHeaplessTree::<BRANCH_FACTOR, PEAK_HEIGHT_1, 5, StdHash>::try_from(
//             &words1.iter().map(|w| w.as_bytes()).collect::<Vec<_>>()
//         )
//         .unwrap();
...
//         let cmt2 = CompactableHeaplessTree::<BRANCH_FACTOR, PEAK_HEIGHT_2, 5, StdHash>::try_from(
//             &words2.iter().map(|w| w.as_bytes()).collect::<Vec<_>>()
//         )
//         .unwrap();
